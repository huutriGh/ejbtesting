...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "ejbtesting-ear.ear");

        // Current directory is the root of the "ejbtesting-web" project. Go up one level
        // and enter the "ejbtesting-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../ejbtesting-ejb/target/ejbtesting-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "ejbtesting-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/ejbtesting-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "ejbtesting-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
